<template>
  <div class="fixed inset-0 z-50">
    <div class="modal-overlay absolute inset-0 bg-gray-900/50 backdrop-blur-sm" @click="emit('close')"></div>
    <div class="absolute inset-4 md:inset-10 lg:inset-y-10 lg:left-1/4 lg:right-1/4 bg-white rounded-lg shadow-2xl overflow-hidden flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto p-6">
        <div class="max-w-lg mx-auto">
          <div v-if="recipe.thumbnail" class="w-full aspect-[9/16] max-h-80 rounded-lg overflow-hidden mb-6 bg-gray-100">
          ...
          <p v-if="recipe.author" class="text-gray-600 mb-4 flex items-center gap-2">
          ...
          <div 
            ...
            :class="['mb-4 inline-flex items-center gap-2 px-3 py-2 rounded',
              isToday ? 'bg-gray-900/10 text-gray-900' : isPast ? 'bg-gray-100 text-gray-500' : 'bg-gray-100 text-gray-700']"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-if="recipe.description" class="bg-gray-50 rounded-lg p-4">
            <h4 class="font-medium text-gray-900 mb-2">Notes & Description</h4>
      ...
      <div class="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
        <button 
          ...
          class="px-5 py-2.5 bg-red-50 border border-red-200 text-red-600 rounded-lg hover:bg-red-100 transition-colors font-medium"
        >
          Delete
        </button>
        <div class="flex gap-3">
          ...
          <a 
            :href="recipe.url"
            target="_blank"
            class="px-5 py-2.5 bg-gray-900 hover:bg-gray-800 text-white rounded-lg transition-colors font-medium inline-flex items-center gap-2"
          >
            Open in TikTok
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
